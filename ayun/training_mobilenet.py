...
with open(train_path + '/mean_img.pickle', 'rb') as f:
...
datagen = ImageDataGenerator(rescale= 1.0/255, preprocessing_function=sub_mean_img)
...
model_mo = tf.keras.Model(inputs, outputs)
0.0001
model_mo.compile(optimizer = tf.keras.optimizers.Adam(learning_rate = 0.0001), loss = 'categorical_crossentropy',
...
model_mo.evaluate(test_generator, callbacks= board_eval1)
...
board_eval2 = tf.keras.callbacks.TensorBoard(log_dir=log_eval2, 
                                                      histogram_freq=1) # epoch마다 히스토그램 계산
model_mo.evaluate(test_generator, callbacks= board_eval2) 
